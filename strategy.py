...
def get_MACD(df, fp, slowp, sigp):
    macd, macdsignal, macdhist = talib.MACD(df["close"], fastperiod=fp, slowperiod=slowp, signalperiod=sigp)
    return macd, macdsignal, macdhist

def get_BBANDS(df, tp, nbdup, nbddn, mt):
    upperband, middleband, lowerband = talib.BBANDS(df["close"], timeperiod=tp, nbdevup=nbdup, nbdevdn=nbddn, matype=mt)
...
while True:
    try:
        ...
        df = df.drop_duplicates(subset=["time_end"], keep='last').sort_values(by="time_end", ascending=True).reset_index().drop(columns="index")
        df["RSI"] = get_RSI(df, 14)
        df["MACD"], df["MACD_sig"], df["MACD_hist"] = get_MACD(df,12,26,9)
        df["BBupper"], df["BBmiddle"], df["BBlower"] = get_BBANDS(df, 12,2,2,0)
